...
from fmfourier.utils.callbacks import SamplingCallback
from fmfourier.utils.extraction import dict_to_str, get_training_params
from fmfourier.utils.wandb import maybe_initialize_wandb
...
class TrainingRunner:
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.velocity_model: VelocityModule = instantiate(cfg.velocity_model)
        ...
        if isinstance(self.velocity_model, partial):
            training_params = get_training_params(self.datamodule, self.trainer)
            self.velocity_model = self.velocity_model(**training_params)
    ...
    def train(self) -> None:
        assert not (
            self.velocity_model.scale_noise and not self.datamodule.fourier_transform
        ), "You cannot use noise scaling without the Fourier transform."
        self.trainer.fit(model=self.velocity_model, datamodule=self.datamodule)
