...
from fmfourier.sampling.metrics import MetricCollection
from fmfourier.sampling.sampler import FlowMatchingSampler
from fmfourier.utils.extraction import dict_to_str, get_best_checkpoint, get_model_type
from fmfourier.utils.fourier import idft
...
class SamplingRunner:
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.velocity_model = model_type.load_from_checkpoint(
            checkpoint_path=best_checkpoint_path
        )
        if torch.cuda.is_available():
            self.velocity_model.to(device=torch.device("cuda"))

        # Instantiate sampler
        sampler_partial = instantiate(cfg.sampler)
        self.sampler: FlowMatchingSampler = sampler_partial(
    ...
    def sample(self) -> None:
        ...
        X = self.sampler.sample(
            num_samples=self.num_samples, num_sampling_steps=self.num_sampling_steps
        )
