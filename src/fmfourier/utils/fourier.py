...
def localization_metrics(X: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    X_energy = torch.sum(X**2, dim=2, keepdim=True) / (
        EPS + torch.sum(X**2, dim=(1, 2), keepdim=True)
    )
    ...
    X_spec = torch.sum(X_spec, dim=2, keepdim=True) / (
        EPS + torch.sum(X_spec, dim=(1, 2), keepdim=True)
    )
...
def smooth_frequency(X: torch.Tensor, sigma: float) -> torch.Tensor:
    ...
    ub_re = nyquist_freq + 1 if max_len % 2 == 0 else nyquist_freq
    ub_im = ub_re - 1 if max_len % 2 == 0 else ub_re

    # Define Gaussian kernel for each frequency pair
    k = torch.cat(
        (
            torch.arange(0, ub_re, dtype=torch.float32),
            torch.arange(1, ub_im, dtype=torch.float32),
        )
    )
