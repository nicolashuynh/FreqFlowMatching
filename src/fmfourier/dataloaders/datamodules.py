...
from fmfourier.utils.dataclasses import collate_batch
from fmfourier.utils.fourier import dft, localization_metrics, smooth_frequency
from fmfourier.utils.preprocessing import (
    droughts_preprocess,
    mimic_preprocess,
    nasa_preprocess,
    nasdaq_preprocess,
...
class MIMICIIIDatamodule(Datamodule):
    ...
    def setup(self, stage: str = "fit") -> None:
        ...
        if self.smooth_frequency and self.smoother_width > 0.0:
            print(
                "Old time delocalization: ",
                localization_metrics(self.X_train)[0].mean().item(),
            )
            print(
                "Old frequency delocalization: ",
                localization_metrics(self.X_train)[1].mean().item(),
            )
            self.X_train = smooth_frequency(self.X_train, sigma=self.smoother_width)
            self.X_test = smooth_frequency(self.X_test, sigma=self.smoother_width)
            logging.info("Smoothing the frequency domain of the data.")
            X_loc, X_spec_loc = localization_metrics(self.X_train)
            logging.info(f"New time delocalization: {X_loc.mean().item():.3g}")
            print(f"New time delocalization: {X_loc.mean().item():.3g}")
            logging.info(
                f"New frequency delocalization: {X_spec_loc.mean().item():.3g}"
            )
            print(f"New frequency delocalization: {X_spec_loc.mean().item():.3g}")
...
class NASADatamodule(Datamodule):
    ...
    def setup(self, stage: str = "fit") -> None:
        ...
        if self.smooth_frequency and self.smoother_width > 0.0:
            ...
            print(
                f"Old time delocalization: {localization_metrics(self.X_train)[0].mean().item():.3g}"
            )
            print(
                f"Old frequency delocalization: {localization_metrics(self.X_train)[1].mean().item():.3g}"
            )
            self.X_train = smooth_frequency(self.X_train, sigma=self.smoother_width)
            self.X_test = smooth_frequency(self.X_test, sigma=self.smoother_width)
            logging.info("Smoothing the frequency domain of the data.")
